...
adata.uns['discription'] = 'This scRNA-seq dataset contains 1656 cells and 4768 genes from mouse hematopoietic stem and progenitor cell differentiation. A single-cell resolution map of mouse hematopoietic stem and progenitor cell differentiation. Blood 128, e20-31 (2016).'
...
layout = html.Div([
    dbc.Container([
        ...
        dbc.Col(html.H5("Click the following tabs for step-by-step visualization.", className="text-center"), className="mb-5 mt-5"),
        ...
        dbc.Card([
            dbc.CardHeader(
                dbc.Tabs(
                    [
                        dbc.Tab(label='1. Data Selection', tab_id="tab-ds", label_style={"font-size": "20px"},tab_style= {"margin-left":"auto","margin-right":"auto"}),

                        dbc.Tab(label='2. Quality Control', tab_id="tab-qc", label_style={ "font-size": "20px"}, tab_style= {"margin-left":"auto","margin-right":"auto"}),

                        dbc.Tab(label='3. Dimension Reduction', tab_id="tab-dr", label_style={"font-size": "20px"}, tab_style= {"margin-left":"auto","margin-right":"auto"}),

                        dbc.Tab(label='4. Stream Plot', tab_id="tab-sp", label_style={"font-size": "20px"}, tab_style= {"margin-left":"auto","margin-right":"auto"})
                    ],
                    id="card-tabs", card=True, active_tab='tab-ds')
            ),
            dbc.CardHeader(id="card-header"),
            dbc.CardBody(id="card-content",style={"height": "50rem"}),
            dbc.CardFooter("Chen, H., Albergante, L., Hsu, J.Y. et al. Single-cell trajectories reconstruction, exploration and mapping of omics data with STREAM. Nat Commun 10, 1903 (2019).")
...
def tab_content(active_tab):
    if active_tab == "tab-qc":
        fig_qc = st.plot_qc(adata, jitter=0.2, fig_size=(2, 2), return_svg=True)
        return html.Img(src="data:image/svg+xml;base64,{}".format(fig_qc)),card_qc
